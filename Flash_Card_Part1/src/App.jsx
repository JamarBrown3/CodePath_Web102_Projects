...
export function App() {
  ...
  const handleCardClick = () => {
    ...
    setUserClickedCard(!userClickedCard);


  }

  // cards for the flash card app
  const cards = [
    ...
    { id: 8, question: "What is a useContext?", answer: "A way to share data betweeen components without passing props" },
  ...
  return (
    <>
      ...
      <div>
        <button className="blackfowardbutton" onClick={() => setCurrentCardIndex((prevIndex) => (Math.random(prevIndex) * cards.length) | 0)}>
          ➡️
        </button>
        <button className="blackBackwardbutton" onClick={() => setCurrentCardIndex((prevIndex) => (prevIndex - 1 + cards.length) % cards.length)}>
          ⬅️
        </button>
      </div>
    </>
